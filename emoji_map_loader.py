    ...
    def _parse_emoji_map(self) -> Tuple[List, List, Dict]:
        ...
        for y, row in enumerate(self.emoji_render):
            for x, emoji in enumerate(row):
                ...
                # --- [수정된 부분 시작] --------------------------------
                # 1. 검은색 네모(⬛)를 발견하면 무조건 'brick' 이모지 객체로 변환
                if emoji == '⬛':
                    obj_config = {
                        'type': 'emoji',
                        'pos': (x, y),
                        'emoji_name': 'brick',   # 여기에 원하는 이모지 이름이나 유니코드(🧱) 입력
                        'color': 'grey',         # 미니그리드 내부 색상 코드(필요시)
                        'can_pickup': False,     # 벽처럼 못 줍게 설정
                        'can_overlap': False,    # 벽처럼 통과 못하게 설정
                        'use_emoji_color': True
                    }
                    # 외벽이 아닌 내부 공간인 경우, 혹은 전체 다 적용하고 싶으면 조건 없이 append
                    objects.append(obj_config)
                    continue  # 아래의 기본 로직은 건너뜀
                # -----------------------------------------------------
