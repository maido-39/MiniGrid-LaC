"""
MiniGrid 환경을 위한 Gym Wrapper (레거시 - 상대 움직임 전용)

⚠️ 이 파일은 레거시 코드입니다. 새로운 프로젝트에서는 minigrid_customenv_emoji.py의
MiniGridEmojiWrapper를 사용하세요 (절대 움직임 지원, 기본값: use_absolute_movement=True).
...
class EmojiObject(WorldObj):
    ...
    def render(self, img):
        ...
        try:
            ...
            # Find src/ directory (go up from legacy/relative_movement to find src/)
...
class CustomRoomEnv(MiniGridEnv):
    ...
    def render(self):
        ...
        try:
            ...
            print(f"Warning: Custom agent image load failed ({e}). Using default rendering.")
            import traceback
            traceback.print_exc()
...
def visualize_environment(wrapper):
    ...
    print("Environment image saved to 'custom_environment.png'.")
    plt.show()
...
def test_environment(wrapper):
    ...
    print(f"Start position: {state['agent_pos']}")
    ...
    print(f"\nTest completed: Total {step_count} steps, Final reward: {reward}")
...
def main():
    ...
    """
    print("=" * 60)
    print("MiniGrid Custom Environment Creation Example")
    print("=" * 60)
    
    # 예제 1: 간단한 방 구조
    print("\n[Example 1] Creating Simple Room Structure")
    print("-" * 60)
    ...
    # 예제 1.5: 이모지 객체 사용 예제
    print("\n[Example 1.5] Using Emoji Objects")
    print("-" * 60)
    ...
    print("\nGrid parsing result (emoji names):")
    ...
    print(f"\nEmoji at position (3, 3): {emoji_at_3_3}")
    
    visualize_environment(emoji_wrapper)
    emoji_wrapper.close()
    
    # 예제 2: 실내 집 환경
    print("\n[Example 2] Creating Indoor House Environment (Hallway, Room, Garage)")
    print("-" * 60)
    ...
    # 예제 3: Wrapper 직접 사용
    print("\n[Example 3] Direct Wrapper Usage")
    print("-" * 60)
    custom_config = {
        'start_pos': (2, 2),
        'goal_pos': (10, 10),
    ...
    # 예제 4: VLM 연동 예시
    print("\n[Example 4] VLM Integration Example")
    print("-" * 60)
    ...
    print(f"Action count: {action_space['n']}")
    print(f"Action list: {action_space['actions']}")
    ...
    for action_str in vlm_actions:
        try:
            ...
            print(f"Action '{action_str}' executed: Reward={reward}, Done={done}")
            if done:
                break
