#!/usr/bin/env python3
"""
Step별 Entropy와 Trust 값을 기록하고 시각화하는 스크립트

사용법:
    python analyze_entropy_trust.py <experiment_log.json 경로>
    
예시:
    python analyze_entropy_trust.py "src/logs_good/Episode_2_2_Test_Entropy/experiment_log.json"
"""
...
def extract_entropy_trust_values(step_data: Dict) -> Tuple[Optional[float], Optional[float], Optional[float], Optional[float]]:
    """
    Step 데이터에서 entropy와 trust 값을 추출
    
    Args:
        step_data: step의 JSON 데이터
        
    Returns:
        (entropy_H_X, entropy_H_X_given_S, entropy_H_X_given_LS, trust_T) 튜플
    """
    entropy_H_X = step_data.get('entropy_H_X')
    entropy_H_X_given_S = step_data.get('entropy_H_X_given_S')
    entropy_H_X_given_LS = step_data.get('entropy_H_X_given_LS')
    trust_T = step_data.get('trust_T')
...
def analyze_experiment_log(json_path: str) -> Dict[str, List]:
    ...
    Returns:
        {'steps': [...], 'H_X': [...], 'H_X_given_S': [...], 'H_X_given_LS': [...], 'trust_T': [...]} 딕셔너리
    """
    ...
    for step_data in data:
        ...
        # Entropy와 Trust 값 추출
        h_x, h_x_s, h_x_ls, trust = extract_entropy_trust_values(step_data)
...
def visualize_entropy_trust(data: Dict[str, List], output_path: Optional[str] = None):
    ...
    plt.title('Step별 Entropy와 Trust', fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def main():
    if len(sys.argv) < 2:
        print("사용법: python analyze_entropy_trust.py <experiment_log.json 경로>")
        sys.exit(1)
    ...
    try:
        ...
        output_txt = output_dir / f"entropy_trust_{json_file.stem}.txt"
        ...
        # 시각화
        output_png = output_dir / f"entropy_trust_{json_file.stem}.png"
