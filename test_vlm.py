...
USER_PROMPT = "Describe what you see in this image. Be specific about colors, shapes, and any objects present."
...
def test_vlm_model(model_config, image, system_prompt, user_prompt):
    ...
    try:
        ...
        if handler_type == "openai":
            ...
            handler = QwenHandler(
                model=model,
                api_type=model_config.get("api_type", "dashscope"),
                temperature=model_config.get("temperature", 0.0),
                max_tokens=model_config.get("max_tokens", 1000),
            )
        ...
        if handler_type == "openai":
            ...
            print("  pip install dashscope  # 또는 pip install transformers torch")
        elif handler_type == "gemma":
