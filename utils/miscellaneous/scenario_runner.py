...
class Scenario2Experiment:
    ...
    def vlm_gen_feedback(self, system_prompt: str, user_feedback: str) -> str:
        ...
        feedback_user_prompt = system_prompt_interp(file_name, True, system_prompt, user_feedback)
        ...
        if not raw_response:
            tfu.cprint("Feedback VLM response is empty..")
            return ""
        
        tfu.cprint("Feedback VLM Response Received")
        tfu.cprint("[4-F] Parsing Feedback Response...")
        ...
        if knowledge:
            ...
            if self.prompt_organizer.grounding:
                tfu.cprint(self.prompt_organizer.grounding)
            else:
                tfu.cprint("(None)")
            tfu.cprint("=" * 80)
    ...
    def initialize(self):
        ...
        tfu.cprint("=" * 60)
        tfu.cprint("Scenario 2: VLM Control Experiment (Absolute Coordinate Movement Version)")
        tfu.cprint("=" * 60)
        ...
        tfu.cprint(f"  Map file: {self.json_map_path}")
        self.wrapper = load_emoji_map_from_json(self.json_map_path)
        self.wrapper.reset()
        
        self.state = self.wrapper.get_state()
        tfu.cprint(f"Agent Start Position: {self.state['agent_pos']}")
        tfu.cprint(f"Agent Direction: {self.state['agent_dir']}")
        ...
        tfu.cprint(f"  - Available actions: {action_space['actions']}")
        
        tfu.cprint("\n[2] VLM initialization complete")
        tfu.cprint("\n" + "=" * 60)
        tfu.cprint("Experiment Start")
        tfu.cprint("=" * 60)
    ...
    def run_step(self):
        ...
        if self.user_prompt is None:
            tfu.cprint("[Warning] No user prompt provided. Using empty prompt.")
        ...
        if has_feedback:
            ...
            tfu.cprint("\n[4-1] Feedback processing complete. Proceeding to the next step.")
            return True
        ...
        if len(action_chunk) > 0:
            for i, action in enumerate(action_chunk, 1):
                ...
                tfu.cprint(f"  {marker} [{i}] {action}")
        else:
            tfu.cprint("  (No action)")
        ...
        tfu.cprint("  Spatial Description:")
        if spatial_desc:
            tfu.cprint(f"    {spatial_desc}")
        else:
            tfu.cprint("    (None)")
        
        tfu.cprint("  Task Process:")
        if task_goal or task_status:
            tfu.cprint(f"    Goal: {task_goal if task_goal else '(None)'}")
            tfu.cprint(f"    Status: {task_status if task_status else '(None)'}")
        else:
            tfu.cprint("    (None)")
        
        tfu.cprint("  Previous Action:")
        if prev_action:
            tfu.cprint(f"    {prev_action}")
        else:
            tfu.cprint("    (None)")
